defmodule ExAws.KMSIntegratinTest do
  ...
  if System.get_env("TEST_EX_AWS_KEY_ARN") do
    ...
    test "GenerateDataKey and Decrypt" do
      assert {:ok, %{"CiphertextBlob" => ciphertext, "KeyId" => key_id, "Plaintext" => plaintext}} =
               ExAws.KMS.generate_data_key(key_id) |> ExAws.request()

      {:ok, key} = Base.decode64(plaintext)
      iv = :crypto.strong_rand_bytes(32)

      {encrypt_ciphertext, encrypt_ciphertag} =
        :crypto.block_encrypt(:aes_gcm, key, iv, {"", "hello"})

      assert {:ok, %{"KeyId" => decrypt_key_id, "Plaintext" => decrypt_plaintext}} =
               ExAws.KMS.decrypt(ciphertext) |> ExAws.request()
    ...
    test "Encrypt" do
      ...
      assert {:ok, %{"CiphertextBlob" => ciphertext, "KeyId" => key_arn}} =
               ExAws.KMS.encrypt(key_id, plaintext) |> ExAws.request()
    ...
    test "GenerateDataKeyWithoutPlaintext" do
      assert {:ok, %{"CiphertextBlob" => _blob, "KeyId" => _key_id}} =
               key_id |> ExAws.KMS.generate_data_key_without_plaintext() |> ExAws.request()
    end
    ...
    test "GetKeyRotationStatus" do
      assert {:ok, %{"KeyRotationEnabled" => bool}} =
               key_id |> ExAws.KMS.get_key_rotation_status() |> ExAws.request()

      assert is_boolean(bool)
    end

    test "RotateKeyOnDemand" do
      assert {:ok, %{"KeyId" => _}} = key_id |> ExAws.KMS.rotate_key_on_demand() |> ExAws.request()
    end
    ...
    test "ListGrants" do
      assert {:ok, %{"Grants" => _, "Truncated" => bool}} =
               key_id |> ExAws.KMS.list_grants() |> ExAws.request()

      assert is_boolean(bool)
    end

    test "ListKeyPolicies" do
      assert {:ok, %{"PolicyNames" => _policy_name, "Truncated" => bool}} =
               key_id |> ExAws.KMS.list_key_policies() |> ExAws.request()
    ...
    test "ReEncrypt" do
      ...
      assert {:ok,
              %{
                "CiphertextBlob" => re_encrypted_ciphertext,
                "KeyId" => re_encrypted_key_id,
                "SourceKeyId" => old_key_id
              }} = ExAws.KMS.re_encrypt(ciphertext, key_id) |> ExAws.request()
